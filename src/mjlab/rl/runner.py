...
def wandb_logging_active(logger) -> bool:
  """True if the run logs to W&B.

  rsl_rl's ``Logger.__init__`` rewrites ``logger_type`` from the configured
  ``"wandb"`` to the writer class name ``"WandbLogWriter"``, so comparing
  against ``"wandb"`` alone silently disables every upload gated on it (the
  export succeeds locally and the wandb.save never runs). Accept both.
  """
  return getattr(logger, "logger_type", None) in ("wandb", "WandbLogWriter")
