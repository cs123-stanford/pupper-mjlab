...
class UniformVelocityCommand(CommandTerm):
  ...
  def __init__(self, cfg: UniformVelocityCommandCfg, env: ManagerBasedRlEnv):
    ...
    # Heading-hold mode (cfg.heading_hold_kp): emitted-command buffer with
    # the yaw channel replaced by the P-correction, and the held target.
    self._hh_cmd_b = torch.zeros(self.num_envs, 3, device=self.device)
    self._hh_target = torch.zeros(self.num_envs, device=self.device)
    self._hh_prev_active = torch.zeros(
  ...
  def command(self) -> torch.Tensor:
    if self.cfg.heading_hold_kp is not None:
      return self._hh_cmd_b
    return self.vel_command_b

  def reset(self, env_ids: torch.Tensor | slice | None) -> dict[str, float]:
    # Re-arm the heading hold; the target recaptures on the activation edge
    # in compute(), when the freshly reset root pose is in the sim data.
    assert isinstance(env_ids, torch.Tensor)
    self._hh_prev_active[env_ids] = False
  ...
  def create_gui(
    ...
    # Floor each axis at the "Max" slider's own minimum: a task can pin a
    # command range to (0, 0) -- the Pupper jump task pins all three -- and a
    # 0.0 initial value would trip viser's max >= value >= min assert. The
    # joystick only writes to envs when "Enable" is checked, so offering a
    # small manual range on a pinned axis is harmless.
    axes = [
      ("lin_vel_x", max(ranges.lin_vel_x[1], 0.1)),
      ("lin_vel_y", max(ranges.lin_vel_y[1], 0.1)),
      ("ang_vel_z", max(ranges.ang_vel_z[1], 0.1)),
    ]
    sliders: list = []
  ...
  def compute(self, dt: float) -> None:
    ...
    # Heading hold: mirror of the deploy-side loop (IMU yaw captured at the
    # quiet edge, clipped P fed back as the yaw command). While walking with
    # a quiet commanded yaw, hold the heading captured when the yaw went
    # quiet; a commanded turn or a stop disengages and re-arms. Gating on
    # the walking command keeps corrections from producing yaw activity at
    # standstill.
    if self.cfg.heading_hold_kp is not None:
      heading = self.robot.data.heading_w
      raw = self.vel_command_b
      active = (raw[:, 2].abs() < self.cfg.heading_hold_yaw_threshold) & (
        torch.norm(raw[:, :2], dim=1) > self.cfg.heading_hold_walk_threshold
      )
      capture = active & ~self._hh_prev_active
      self._hh_target = torch.where(capture, heading, self._hh_target)
      self._hh_prev_active = active
      err = wrap_to_pi(self._hh_target - heading)
      clip = self.cfg.heading_hold_clip
      corr = torch.clamp(self.cfg.heading_hold_kp * err, -clip, clip)
      self._hh_cmd_b[:] = raw
      self._hh_cmd_b[:, 2] = torch.where(active, corr, raw[:, 2])
...
class UniformVelocityCommandCfg(CommandTermCfg):
  ...
  heading_hold_kp: float | None = None
  """When set, hold heading while walking with a quiet commanded yaw: the
  heading at the moment the yaw command drops below
  ``heading_hold_yaw_threshold`` is captured, and the emitted yaw command
  becomes ``clip(kp * heading_error, +-heading_hold_clip)`` until a turn is
  commanded or the linear command goes quiet. Mirrors the deploy-side
  correction (IMU yaw captured at the quiet edge, clipped P fed back as the
  yaw command), so the policy sees the same closed-loop command profile the
  robot receives; a frozen yaw-rate-tracking policy needs no retraining.
  None (default) disables."""
  heading_hold_clip: float = 0.3
  """Max magnitude of the corrective yaw command [rad/s]."""
  heading_hold_yaw_threshold: float = 0.1
  """Commanded |yaw| below this counts as quiet."""
  heading_hold_walk_threshold: float = 0.05
  """Corrections only apply while the linear command norm exceeds this, so
  a hold at standstill cannot produce yaw activity."""
